...
`Suifengpiao\Admin\Widgets\Box`用来生成box组件：
...
`Suifengpiao\Admin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`Suifengpiao\Admin\Widgets\Form`类用来快速构建表单：
...
`Suifengpiao\Admin\Widgets\Table`类用来生成表格：
